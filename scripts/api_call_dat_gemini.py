...
client = genai.Client(api_key='AIzaSyDUvfrfIAkTvgqOAUap1Tkk95_Hyu0dlKs', http_options={'api_version':'v1alpha'})



def generate_gemini_response(text, temp):
    config = {'thinking_config': {'include_thoughts': False}, 'temperature': temp}
...
def main(filename, file_path="./", strategy="none", temp=None, iter_nb="0"):
    ...
    for iterat in range(0, 500):
        ...
        try:
            response = generate_gemini_response(strategies[strategy], temp)
